...
To edit the file in |VSC|, use the nRF Kconfig extension.
This extension organizes the Kconfig option in a hierarchical list and lets you select and deselect the desired options.
To save the changes made using the nRF Kconfig extension, click the :guilabel:`Save` button.
...
To locate a specific configuration option, use the filter (:guilabel:`Jump To Item` in nRF Kconfig or :guilabel:`Jump to` in menuconfig and guiconfig).
...
* Edit the Kconfig options using the `nRF Kconfig`_ extension.
  To save changes permanently to the :file:`prj.conf` file, click the :guilabel:`Save to file` button.
...
See `Building an application`_ in the nRF Connect for Visual Studio Code documentation.
